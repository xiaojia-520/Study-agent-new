...
class SessionTranscriptRefineService:
    def __init__(
        ...
        *,
        ...
        batch_record_limit: int | None = None,
    ) -> None:
        self.store = store
        ...
        self.batch_record_limit = max(1, int(batch_record_limit)) if batch_record_limit is not None else None
        self._runtime = None
    ...
    def _build_batches(self, records: Sequence[Mapping[str, Any]]) -> list[list[Mapping[str, Any]]]:
        ...
        if not materialized:
            return []
        if self.batch_record_limit is None:
            return [materialized]
        return [
            materialized[index : index + self.batch_record_limit]
            for index in range(0, len(materialized), self.batch_record_limit)
        ]
